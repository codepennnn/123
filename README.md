...
ORDER BY
  ...
  private bool Check()
  {
      ...
      foreach (var item in licenseTotals)
      {
          ...
          double maxAllowedMandays = capacity * item.Value.daysWorked;
...
i want one more validation that i have query that return basicrate in which add parameter of workorder year month and vendor code
  this basic rate should be multiple by  total mandays of each workorder per license license then check if sum of gross male female for said license of each workorder > that rate
